...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
		...
		if (AttackStates.Contains (playerState.currentState) && !isDead) {
			...
			if (action == "Punch" && buttonState == BUTTONSTATE.PRESS && playerState.currentState != UNITSTATE.PUNCH && playerState.currentState != UNITSTATE.KICK && isGrounded) {
                ...
                if (is_used_slow_camera)
                {
                    if (attackNum == 4)
                    {
                        float temp = slow_cam.distanceToTarget;
                        slow_cam.gameObject.GetComponent<Animator>().Rebind();
                        slow_cam.gameObject.GetComponent<Animator>().enabled = true;
                        GameObject.FindGameObjectWithTag("yes").GetComponent<AudioSource>().Play();
                        Time.timeScale = 0.3f;
                    }
                    else
                    {
                        Time.timeScale = 1.0f;
                        slow_cam.gameObject.GetComponent<Animator>().Rebind();
                        slow_cam.gameObject.GetComponent<Animator>().enabled = false;
                    }
                }
	...
	public void equipWeapon(Weapon weapon){
		...
		if(weapon.playerHandPrefab != null) {
			...
			PlayerWeapon.transform.localScale = new Vector3(100, 100, 100);
			PlayerWeapon.transform.localEulerAngles = new Vector3(180, 0, 0);
