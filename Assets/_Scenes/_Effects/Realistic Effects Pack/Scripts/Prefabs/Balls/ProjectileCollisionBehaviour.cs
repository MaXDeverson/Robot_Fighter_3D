  ...
  void OnDisable()
  {
    ...
    private void InitializeDefault()
    {
        ...
        if (!effectSettings.UseMoveVector)
        {
            ...
            if (tTarget != null)
            {
                // Debug.Log("taget"+tTarget.transform.eulerAngles);
                ...
                if (forwardDirection.y <= 2)
                {
                    forwardDirection = new Vector3(forwardDirection.x, 5, forwardDirection.z);
                    Debug.Log("forwaedisrection" + forwardDirection.y);
                }
    ...
    private void FixedUpdate()
    {
        ...
        effectSettings.CollisionEnter += (n, e) =>
        {
         ...
         //Debug.Log("object name is " + e.Hit.transform.name);
                if (e.Hit.transform.gameObject.tag == "Enemy")
                {
                    //e.Hit.transform.gameObject.GetComponent<AI_Contoller>().Health -= 0.3f;
                   // e.Hit.transform.gameObject.GetComponent<AI_Contoller>().EnergyBar.GetComponent<imag>().BarProgress = e.Hit.transform.gameObject.GetComponent<AI_Contoller>().Health / 100;
                }
                if (e.Hit.transform.gameObject.tag == "car")
                {
                    ////Debug.Log("I am a car");
                    //if (e.Hit.transform.gameObject.GetComponent<moving_car>())
                    //{
                    //    e.Hit.transform.gameObject.GetComponent<moving_car>().Die();
                    //}
                }
                if (e.Hit.transform.gameObject.tag == "TrafficCar")
                {
                   //Debug.Log("I am a car cars");
                   // if (e.Hit.transform.gameObject.GetComponent<car_destroy>())
                   // {
                   //     e.Hit.transform.gameObject.GetComponent<car_destroy>().destoy_car();
                   // }
                }
            } 
